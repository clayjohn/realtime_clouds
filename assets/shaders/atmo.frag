...
const vec2 resolution = vec2(128.0);
uniform float time;
uniform mat4 MVPM;
uniform int check;
out vec4 color;
...
void main()
{
	...
	if (check_pos(gl_FragCoord.xy/4.0, 4.0)!=check&&true==false){
		...
		if (point_cam.y>0.0) {
	// sun
			mat3 rot = rotate_around_x(-abs(sin(u_time / 20.)) * 180.);
			sun_dir *= rot;
      ...
      col.xyz = get_incident_light(ray);
			col.a=1.0;
		} else {
			discard;
		}
